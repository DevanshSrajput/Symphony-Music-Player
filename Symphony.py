...
dpg.create_context() 
dpg.create_viewport(title="Symphony Music Player",large_icon="icon.ico",small_icon="icon.ico")
pygame.mixer.init()
global state
state=None
...
with dpg.window(tag="main",label="window title"):
	...
	with dpg.group(horizontal=True):
		with dpg.child_window(width=200,tag="sidebar"):
			dpg.add_text("Symphony",color=(137, 142, 255))
			dpg.add_text("Build by Devansh Singh")
			dpg.add_spacer(height=2)
			dpg.add_button(label="Support",width=-1,height=23,callback=lambda:webbrowser.open(url="https://github.com/DevanshSrajput"))
